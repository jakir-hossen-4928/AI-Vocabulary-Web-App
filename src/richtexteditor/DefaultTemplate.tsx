    ...
    <>
      <div className="lexkit-toolbar">
        ...
        <div className="lexkit-toolbar-section">
          <button type="button" onClick={() => commands.toggleBold()} className={`lexkit-toolbar-button ${activeStates.bold ? "active" : ""}`} title="Bold (Ctrl+B)"><Bold size={16} /></button>
          <button type="button" onClick={() => commands.toggleItalic()} className={`lexkit-toolbar-button ${activeStates.italic ? "active" : ""}`} title="Italic (Ctrl+I)"><Italic size={16} /></button>
          <button type="button" onClick={() => commands.toggleUnderline()} className={`lexkit-toolbar-button ${activeStates.underline ? "active" : ""}`} title="Underline (Ctrl+U)"><Underline size={16} /></button>
          <button type="button" onClick={() => commands.toggleStrikethrough()} className={`lexkit-toolbar-button ${activeStates.strikethrough ? "active" : ""}`} title="Strikethrough"><Strikethrough size={16} /></button>
          <button type="button" onClick={() => commands.formatText("code")} className={`lexkit-toolbar-button ${activeStates.code ? "active" : ""}`} title="Inline Code"><Code size={16} /></button>
          <button type="button" onClick={() => activeStates.isLink ? commands.removeLink() : commands.insertLink()} className={`lexkit-toolbar-button ${activeStates.isLink ? "active" : ""}`} title={activeStates.isLink ? "Remove Link" : "Insert Link"}>
        ...
        {hasExtension("blockFormat") && (
          <div className="lexkit-toolbar-section">
            ...
            {hasExtension("code") && (
              <button type="button" onClick={() => commands.toggleCodeBlock()} className={`lexkit-toolbar-button ${activeStates.isInCodeBlock ? "active" : ""}`} title="Code Block"><Terminal size={16} /></button>
            )}
          </div>
        )}
        ...
        {hasExtension("list") && (
          <div className="lexkit-toolbar-section">
            <button type="button" onClick={() => commands.toggleUnorderedList()} className={`lexkit-toolbar-button ${activeStates.unorderedList ? "active" : ""}`} title="Bullet List"><List size={16} /></button>
            <button type="button" onClick={() => commands.toggleOrderedList()} className={`lexkit-toolbar-button ${activeStates.orderedList ? "active" : ""}`} title="Numbered List"><ListOrdered size={16} /></button>
            <button type="button" onClick={() => commands.indentList()} className="lexkit-toolbar-button" title="Indent"><Indent size={14} /></button>
            <button type="button" onClick={() => commands.outdentList()} className="lexkit-toolbar-button" title="Outdent"><Outdent size={14} /></button>
          </div>
        )}
        ...
        {hasExtension("horizontalRule") && (
          <div className="lexkit-toolbar-section">
            <button type="button" onClick={() => commands.insertHorizontalRule()} className="lexkit-toolbar-button" title="Insert Horizontal Rule"><Minus size={16} /></button>
          </div>
        )}
        ...
        {hasExtension("table") && (
          <div className="lexkit-toolbar-section">
            <button type="button" onClick={() => setShowTableDialog(true)} className="lexkit-toolbar-button" title="Insert Table (Ctrl+Shift+T)"><TableIcon size={16} /></button>
          </div>
        )}
        ...
        {hasExtension("image") && (
          <div className="lexkit-toolbar-section">
            <Dropdown
              trigger={<button type="button" className={`lexkit-toolbar-button ${activeStates.imageSelected ? "active" : ""}`} title="Insert Image"><ImageIcon size={16} /></button>}
              isOpen={showImageDropdown}
              onOpenChange={setShowImageDropdown}
            >
              <button type="button" className="lexkit-dropdown-item" onClick={() => { handlers.insertFromUrl(); setShowImageDropdown(false); }}><Link size={16} /> From URL</button>
              <button type="button" className="lexkit-dropdown-item" onClick={() => { handlers.insertFromFile(); setShowImageDropdown(false); }}><Upload size={16} /> Upload File</button>
            </Dropdown>
        ...
        {hasExtension("htmlEmbed") && (
          <div className="lexkit-toolbar-section">
            <button type="button" onClick={() => commands.insertHTMLEmbed()} className={`lexkit-toolbar-button ${activeStates.isHTMLEmbedSelected ? "active" : ""}`} title="Insert HTML Embed"><FileCode size={16} /></button>
            {activeStates.isHTMLEmbedSelected && (
              <button type="button" onClick={() => commands.toggleHTMLPreview()} className="lexkit-toolbar-button" title="Toggle Preview/Edit">
...
function ModeTabs({ mode, onModeChange }: { mode: EditorMode; onModeChange: (mode: EditorMode) => void }) {
  return (
    <div className="lexkit-mode-tabs">
      <button type="button" className={`lexkit-mode-tab ${mode === "visual" ? "active" : ""}`} onClick={() => onModeChange("visual")}>Visual</button>
      <button type="button" className={`lexkit-mode-tab ${mode === "html" ? "active" : ""}`} onClick={() => onModeChange("html")}>HTML</button>
      <button type="button" className={`lexkit-mode-tab ${mode === "markdown" ? "active" : ""}`} onClick={() => onModeChange("markdown")}>Markdown</button>
    </div>
  );
}
...
function EditorContent({
  ...
  useEffect(() => {
    ...
    return editor.registerUpdateListener(({ editorState }) => {
      editorState.read(() => {
        const markdown = $convertToMarkdownString(extensions as any);
        onChange(markdown);
      });
    });
  }, [editor, onChange]);
  ...
  return (
    <>
      ...
      {!readOnly && <CommandPalette isOpen={commandPaletteOpen} onClose={() => setCommandPaletteOpen(false)} commands={commandsToCommandPaletteItems(commands)} />}
    </>
  );
}
...
export const DefaultTemplate = forwardRef<DefaultTemplateRef, DefaultTemplateProps>(({ className, onReady, onChange, readOnly = false }, ref) => {
  ...
  useEffect(() => {
    imageExtension.configure({
      ...
      resizable: !readOnly,
      pasteListener: { insert: !readOnly, replace: !readOnly },
      debug: false,
    });
  }, [readOnly]);
